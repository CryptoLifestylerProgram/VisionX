...
*Setup instructions for a Gitian build of VisionX Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the VisionX
...
- [Building VisionX Core](#building-visionX-core)
...
Clone the git repositories for VisionX Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/visionXcoin/visionXcoin
```
...
in the VisionX Core repository under 'Fetch and build inputs' to install sources which require
...
To build VisionX Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the VisionX Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/visionX/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/visionXcoin/visionXcoin
...
For example:
```bash
URL=https://github.com/crowning-/visionX.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit visionX=${COMMIT} --url visionX=${URL} ../visionX/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit visionX=${COMMIT} --url visionX=${URL} ../visionX/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit visionX=${COMMIT} --url visionX=${URL} ../visionX/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the visionX git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../visionX/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/visionXcoin.git
SIGPATH=/some/root/path/visionXcoin-detached-sigs.git

./bin/gbuild --url visionX=${BTCPATH},signature=${SIGPATH} ../visionX/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/visionX-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/visionX-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/visionX-osx-build.assert
```
...
[visionX/gitian.sigs](https://github.com/visionXcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
