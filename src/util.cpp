...
const char * const BITCOIN_CONF_FILENAME = "visionX.conf";
const char * const BITCOIN_PID_FILENAME = "visionXd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "visionX" is a composite category enabling all VisionX-related debug output
            if(ptrCategory->count(string("visionX"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VisionXCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VisionXCore
    // Mac: ~/Library/Application Support/VisionXCore
    // Unix: ~/.visionXcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VisionXCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VisionXCore";
#else
    // Unix
    return pathRet / ".visionXcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty visionX.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("visionX.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override visionX.conf
